  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  // target_rune_sub_ = this->create_subscription<buff_interfaces::msg::Rune>(
  ...
  aim_sync_ = std::make_unique<AimSync>(aim_syncpolicy(1500), aim_sub_, aim_time_info_sub_);
  aim_sync_->registerCallback(
    std::bind(&RMSerialDriver::sendArmorData, this, std::placeholders::_1, std::placeholders::_2));

  buff_sync_ = std::make_unique<BuffSync>(buff_syncpolicy(3000), rune_sub_, buff_time_info_sub_);
  ...
  //   500ms, [this]() {
  //       if (!aim_sync_->getLastEvents().empty() && !aim_sync_->getLastEvents().back().wasSuccessful()) {
  //           RCLCPP_WARN(this->get_logger(), "消息同步失败：最近的同步尝试没有成功");
  //       }
  //   });
...
void RMSerialDriver::DecisionCallBack(Protocol_UpDataReferee_t & data)
{
  ...
  // decision.red_3_robot_hp = data.red_3_robot_hp;  // TEST
...
void RMSerialDriver::sendBuffData( const buff_interfaces::msg::Rune::ConstSharedPtr rune,
                                   const buff_interfaces::msg::TimeInfo::ConstSharedPtr time_info)
{
  ...
  try {
    ...
    if (rune->w == 0) {
      ...
      rclcpp::Time obj_time = rune->header.stamp;
      int obj_time_ms = (obj_time.seconds() * 1000);
      int offset = (rune->t_offset + obj_time_ms  % T) % T;
        // int offset = (rune->t_offset + time_info->time % T) % T; //debug - -> +
      if (offset < 0) {
