...
private:
 ...
 //（自瞄）视觉->电控回调函数
  ...
  // BUff
  // rclcpp::Subscription<buff_interfaces::msg::Rune>::SharedPtr target_rune_sub_;
  void sendBuffData(const buff_interfaces::msg::Rune::ConstSharedPtr rune,
  ...
  */
  ...
  void DecisionCallBack(Protocol_UpDataReferee_t & data);  //（决策）电控->视觉回调函数
    // 电控->视觉发布者
  //rclcpp::Publisher<rm_decision_interfaces::msg::CvDecision>::SharedPtr decision_pub_;
  rclcpp::Publisher<rm_decision_interfaces::msg::CvDecision>::SharedPtr robot_status_pub_;
  rclcpp::Publisher<rm_decision_interfaces::msg::CvDecision>::SharedPtr game_status_pub_;
  rclcpp::Publisher<rm_decision_interfaces::msg::CvDecision>::SharedPtr robot_hp_pub_;
};
}  // namespace rm_serial_driver
