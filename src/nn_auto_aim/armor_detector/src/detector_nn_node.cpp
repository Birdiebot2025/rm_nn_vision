...
void ArmorDetectorNode::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr img_msg)
{
  ...
  if (!pnp_solver_) {
    RCLCPP_WARN(get_logger(), "PnP solver not initialized!");
    return;
  }
...
std::unique_ptr<auto_aim::YOLO> ArmorDetectorNode::inityolo()
{
  detect_color = declare_parameter("detect_color", RED);
  auto pkg_path = ament_index_cpp::get_package_share_directory("armor_detector");
  auto config_path = pkg_path;
  auto yolo = std::make_unique<auto_aim::YOLO>(config_path, false);
  return yolo;
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  detect_color = get_parameter("detect_color").as_int();
  // detector_->classifier->threshold = get_parameter("classifier_threshold").as_double();
  auto allarmors = yolo_->detect(img);
  for (const auto &narmor : allarmors) {
      if (narmor.color != detect_color) continue;

      rm_auto_aim::Armor armor;
      armor.left_light.top = narmor.points[0];
      armor.right_light.top = narmor.points[1];
      armor.right_light.bottom = narmor.points[2];
      armor.left_light.bottom = narmor.points[3];
  ...
  if (debug_) {
    ...
    for (const auto &armor : armors) {
        ...
        cv::line(img, armor.right_light.bottom, armor.left_light.bottom, cv::Scalar(0,255,0), 2);
        cv::line(img, armor.left_light.bottom, armor.left_light.top, cv::Scalar(0,255,0), 2);

        // cv::putText(img, armor.classfication_result,
        //             armor.left_light.top, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,0,0), 2);
    }
    // Draw camera center
