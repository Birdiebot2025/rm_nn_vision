...
void Tracker::update(const buff_interfaces::msg::BladeArray::SharedPtr & blades_msg, double dt)
{
  ...
  if (!blades_msg->blades.empty()) {
    ...
    center_xoy_diff = sqrt(
      ...
      pow(tracked_blade.center_position.z - predict_blade.center_position.z, 2)
      );
    ...
    if (center_xoy_diff < max_match_center_xoy_ || tracker_state == DETECTING) {
      ...
      std::cout<<center_xoy_diff<<std::endl;
    }
  }
...
// theta_diff = measurement - prediction
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("buff_tracker"), "Blade jump to illegal angle, use predict");
  return false;
}
