...
std::vector<Blade> Detector::Detect(cv::Mat & src_img)
{
  ...
  //   {
  //     if(i!=2)
  //     points.push_back(blade.kpt[i]);
  //   }
  //   correctKeypoints(points);
  //   int j = 0;
  //   for(int i = 0; i < 5; i++)
  //   {
  //       if(i != 2)
  //           blade.kpt[i] = points[j++];
  //   }
  // }

  return blade_array_;
}
// void Detector::correctKeypoints(std::vector<cv::Point2f>& points)
// {
//     // 计算质心
...
//     for (const auto& point : points)
//     {
//         vectors.push_back(point - centroid);
//     }
...
//         avg_length += length;
//     }
//     avg_length /= lengths.size();

//     // 生成理想正方形的顶点，按照左上角、左下角、右上角、右下角的顺序
//     std::vector<cv::Point2f> ideal_points = {
//         cv::Point2f(centroid.x - avg_length, centroid.y - avg_length),
//         cv::Point2f(centroid.x - avg_length, centroid.y + avg_length),
//         cv::Point2f(centroid.x + avg_length, centroid.y - avg_length),
//         cv::Point2f(centroid.x + avg_length, centroid.y + avg_length)
//     };
...
//     // 计算仿射变换矩阵
//     cv::Mat M = getPerspectiveTransform(points, ideal_points);

//     // 应用仿射变换
//     std::vector<cv::Point2f> corrected_points;
//     perspectiveTransform(points, corrected_points, M);
...
void Detector::correctKeypoints(std::vector<cv::Point2f>& points)
{
    if (points.size() != 4)
    {
        throw std::invalid_argument("points vector must contain exactly 4 points");
    }
    ...
    // 计算关键点到质心的矢量和角度
    std::vector<std::pair<cv::Point2f, float>> point_angle_pairs;
    for (const auto& point : points)
    {
        cv::Point2f vector = point - centroid;
        float angle = atan2(vector.y, vector.x);
        point_angle_pairs.push_back(std::make_pair(point, angle));
    }

    // 按角度排序点，确保顺序为左上角、左下角、右上角、右下角
    std::sort(point_angle_pairs.begin(), point_angle_pairs.end(), [](const std::pair<cv::Point2f, float>& a, const std::pair<cv::Point2f, float>& b) {
        return a.second < b.second;
    });

    // 重新排列后的点
    std::vector<cv::Point2f> sorted_points;
    for (const auto& pair : point_angle_pairs)
    {
    ...
    {
        lengths.push_back(norm(point - centroid));
    }
    float avg_length = std::accumulate(lengths.begin(), lengths.end(), 0.0f) / lengths.size();

    // 生成根据排序后的角度旋转的理想正方形的顶点
    std::vector<cv::Point2f> ideal_points;
    for (const auto& pair : point_angle_pairs)
    {
        float angle = pair.second;
        ideal_points.push_back(cv::Point2f(centroid.x + avg_length * cos(angle), centroid.y + avg_length * sin(angle)));
    }

    // 计算透视变换矩阵
    cv::Mat M = cv::getPerspectiveTransform(sorted_points, ideal_points);

    // 应用透视变换
    std::vector<cv::Point2f> corrected_points(4);
    cv::perspectiveTransform(sorted_points, corrected_points, M);
  ...
  ov::Tensor & output, float conf_thres, float iou_thres, int nc, cv::Size img_size)
{
  ...
  int bs = output.get_shape()[0];  // batch size 1
  int nm = output.get_shape()[1];  // number of masks 8400
  int ml = output.get_shape()[2];  // mask length 16
  // debug
// std::cout<<"bs : "<< bs<<" nm : "<< nm<<" ml:<<"<<ml<<std::endl;
...
void Detector::draw_blade(cv::Mat & img)
{
  ...
  for (size_t i = 0; i < blade_array_.size(); ++i) {
    ...
    for(int j = 0;j<4;j++)
    {
      cv::Point center(blade_array_[i].kpt[kpt_idx[j]].x, blade_array_[i].kpt[kpt_idx[j]].y);
      cv::circle(img, center, 8, cv::Scalar(255, 255, 0), -1);

      std::string label = "Pt" + std::to_string(kpt_idx[j]); 
